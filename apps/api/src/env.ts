import { z } from "zod";
import "dotenv/config";

const envSchema = z.object({
  DATABASE_URL: z.string().url(),
  REDIS_URL: z.string().default("redis://localhost:6379"),
  JWT_SECRET: z.string().min(8),
  ADMIN_SEED_PASSWORD: z.string().min(8).optional(),
  SUPER_ADMIN_SEED_EMAIL: z.string().email().optional(),
  SUPER_ADMIN_SEED_PASSWORD: z.string().min(8).optional(),
  SUPER_ADMIN_SEED_NAME: z.string().min(1).optional(),
  EMPLOYEE_SEED_EMAIL: z.string().email().optional(),
  EMPLOYEE_SEED_PASSWORD: z.string().min(8).optional(),
  EMPLOYEE_SEED_NAME: z.string().min(1).optional(),
  API_PORT: z.coerce.number().default(3001),
  API_HOST: z.string().default("0.0.0.0"),
  CORS_ORIGIN: z.string().default("http://localhost:5173"),
  NODE_ENV: z.enum(["development", "production", "test"]).default("development"),
  // AI / Agent
  OPENROUTER_API_KEY: z.string().optional(),
  AGENT_MODEL: z.string().default("google/gemini-2.5-flash"),
  CHAT_MODEL: z.string().default("qwen/qwen3-coder:free"),
});

export const env = envSchema.parse(process.env);
