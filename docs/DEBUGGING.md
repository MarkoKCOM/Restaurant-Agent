...
Unexpected loyalty claim/reward failures return `LOYALTY_OPERATION_FAILED` as a 500 and log at error level. Loyalty and membership read/update paths also return stable route-specific 500 codes such as `MEMBERSHIP_PROCESSING_LIST_FAILED`, `LOYALTY_REFERRAL_SHARE_FAILED`, `LOYALTY_BALANCE_FAILED`, `LOYALTY_HISTORY_FAILED`, `MEMBERSHIP_SUMMARY_FAILED`, `LOYALTY_REWARDS_LIST_FAILED`, `LOYALTY_CLAIM_VERIFY_FAILED`, `LOYALTY_MESSAGING_PREFERENCES_FAILED`, or `LOYALTY_STAMP_CARD_FAILED`, with guest/restaurant context in the log. Known member-facing conditions still use narrower 400/404/409 codes such as `LOYALTY_INSUFFICIENT_POINTS`, `LOYALTY_REWARD_NOT_FOUND`, `LOYALTY_CLAIM_NOT_FOUND`, or `LOYALTY_CLAIM_NOT_ACTIVE`.
