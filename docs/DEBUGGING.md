...
Growth-only API surfaces should reject starter restaurants with a stable `PACKAGE_GROWTH_REQUIRED` error and include `requiredPackage: "growth"` plus the current `restaurantPackage` when known. Start with campaign or analytics endpoints when checking this guard:
