...
`pnpm debug:artifact <bundle>/manifest.json` reads the bundle's membership debug output and prints the overdue pending engagement-job count, failed engagement-job count, and skipped retention reasons in the top-level summary. Start there before opening `membership-debug-summary.txt` when retention automation looks wrong.
