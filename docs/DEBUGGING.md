...
Guest CRM failures are logged at error level with request IDs and guest/restaurant context. Look for `GUEST_LIST_FAILED`, `GUEST_LOOKUP_FAILED`, `GUEST_RESERVATION_HISTORY_FAILED`, `GUEST_CREATE_FAILED`, `GUEST_FULL_PROFILE_FAILED`, `GUEST_SENTIMENT_HISTORY_FAILED`, `GUEST_AUTO_TAG_FAILED`, `GUEST_PREFERENCES_UPDATE_FAILED`, or `GUEST_UPDATE_FAILED` when dashboard guest profiles, hospitality notes, tags, or sentiment views fail.

Waitlist failures are logged at error level with waitlist/restaurant context. Look for `WAITLIST_ADD_FAILED`, `WAITLIST_LIST_FAILED`, `WAITLIST_LOOKUP_FAILED`, `WAITLIST_OFFER_FAILED`, `WAITLIST_ACCEPT_FAILED`, or `WAITLIST_CANCEL_FAILED` when add/list/offer/accept/cancel workflows fail before checking reservation creation or table availability internals.

Table management failures are logged at error level with table/restaurant context. Look for `TABLE_LIST_FAILED`, `TABLE_CREATE_FAILED`, `TABLE_LOOKUP_FAILED`, `TABLE_UPDATE_FAILED`, or `TABLE_DEACTIVATE_FAILED` when floor setup, capacity, or availability behavior starts failing after table edits.

Restaurant/settings failures are logged at error level with restaurant context. Look for `RESTAURANT_LIST_FAILED`, `RESTAURANT_LOOKUP_FAILED`, `RESTAURANT_UPDATE_FAILED`, `RESTAURANT_DASHBOARD_FAILED`, `RESTAURANT_SUMMARY_FAILED`, `RESTAURANT_TABLE_STATUS_FAILED`, `RESTAURANT_RESET_RESERVATIONS_FAILED`, or `RESTAURANT_TABLES_FAILED` when tenant setup, owner summaries, Today dashboard, or table-status reads fail.

Admin diagnostics failures are logged at error level with stable request IDs. Look for `ADMIN_DIAGNOSTICS_FAILED` when `/api/v1/admin/diagnostics` or the debug bundle cannot build the health report, and `ADMIN_RESTAURANTS_LIST_FAILED` when super-admin tenant listing or owner-delivery readiness cannot load restaurant/admin rows.

Auth failures are logged with stable request IDs. Look for `AUTH_LOGIN_LOOKUP_FAILED`, `AUTH_PASSWORD_VERIFY_FAILED`, or `AUTH_LOGIN_RESPONSE_FAILED` when login fails unexpectedly, and `AUTH_SIGNUP_LOOKUP_FAILED`, `AUTH_PASSWORD_HASH_FAILED`, or `AUTH_SIGNUP_FAILED` when self-serve onboarding cannot create the tenant/admin account.

Unexpected loyalty claim/reward failures return `LOYALTY_OPERATION_FAILED` as a 500 and log at error level. Loyalty and membership read/update paths also return stable route-specific 500 codes such as `MEMBERSHIP_PROCESSING_LIST_FAILED`, `LOYALTY_REFERRAL_SHARE_FAILED`, `LOYALTY_BALANCE_FAILED`, `LOYALTY_HISTORY_FAILED`, `MEMBERSHIP_SUMMARY_FAILED`, `LOYALTY_REWARDS_LIST_FAILED`, `LOYALTY_CLAIM_VERIFY_FAILED`, `LOYALTY_MESSAGING_PREFERENCES_FAILED`, or `LOYALTY_STAMP_CARD_FAILED`, with guest/restaurant context in the log. Guest/claim preflight read failures use codes such as `LOYALTY_BALANCE_GUEST_LOOKUP_FAILED`, `LOYALTY_REWARD_CLAIM_GUEST_LOOKUP_FAILED`, or `LOYALTY_CLAIM_LOOKUP_FAILED` so lookup problems are distinguishable from the downstream loyalty action. Known member-facing conditions still use narrower 400/404/409 codes such as `LOYALTY_INSUFFICIENT_POINTS`, `LOYALTY_REWARD_NOT_FOUND`, `LOYALTY_CLAIM_NOT_FOUND`, or `LOYALTY_CLAIM_NOT_ACTIVE`.

Unexpected reservation failures return reservation-specific 500 codes such as `RESERVATION_AVAILABILITY_FAILED`, `RESERVATION_CREATE_FAILED`, `RESERVATION_LIST_FAILED`, `RESERVATION_UPDATE_FAILED`, `RESERVATION_NO_SHOW_FAILED`, or `RESERVATION_CANCEL_FAILED`, with restaurant/reservation context in the log. Expected booking constraints still keep narrower 4xx codes such as `RESERVATION_NO_TABLE_COMBINATION` or `RESERVATION_INVALID_STATUS_TRANSITION`.
