...
The API reliability smoke creates a future-dated challenge to verify launch windows are respected, creates a short-lived current visit-count challenge, completes a test reservation, verifies that the guest challenge progress is created and completed, then deactivates both smoke challenges. This catches regressions where future challenges leak early or the retention/gamification stage stops running even though the reservation itself completed successfully.
