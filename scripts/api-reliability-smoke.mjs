...
const baseUrl = (process.argv[2] || process.env.OPENSEAT_API_URL || "http://localhost:3001").replace(/\/$/, "");
const adminEmail = process.env.ADMIN_EMAIL || "admin@bff.co.il";
const adminPassword = process.env.ADMIN_SEED_PASSWORD || process.env.OPENSEAT_ADMIN_PASSWORD;

if (!adminPassword) {
  console.error("Missing ADMIN_SEED_PASSWORD or OPENSEAT_ADMIN_PASSWORD in environment");
  process.exit(1);
}
