...
- [x] 8.2 Implement challenges — owner creates time-limited challenges, track progress per guest, award on completion
...
- [x] 8.5 Implement lucky spin — configurable prize pool with weighted probabilities, trigger every Nth visit, WhatsApp reward delivery
...
- [x] 10.4 Implement campaign stats tracking — sent, delivered, read, replied counts
- [x] 10.5 Implement campaign rate limiting — max 2/week, 4/month per guest
- [x] 10.6 Implement opt-out handling — "STOP" keyword detection, immediate removal from campaigns
...
- [x] 13.20 Classify agent tool-call failures — malformed LLM tool arguments and tool execution failures appear as distinct toolTrace error codes instead of generic agent failures
