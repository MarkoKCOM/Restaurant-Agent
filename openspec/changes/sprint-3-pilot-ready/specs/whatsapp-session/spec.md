...
- REQ-3: `startSession` initializes a Baileys `makeWASocket` connection. Auth state is loaded from and saved to `/root/openseat/.whatsapp-session/<restaurantId>/` using Baileys' `useMultiFileAuthState`.
...
- REQ-10: Session state directory `/root/openseat/.whatsapp-session/` is created automatically if it does not exist. This path must be in `.gitignore`.
...
- AC-5: Auth state files are written to `/root/openseat/.whatsapp-session/<restaurantId>/`.
...
- Session state path: `/root/openseat/.whatsapp-session/<restaurantId>/`
